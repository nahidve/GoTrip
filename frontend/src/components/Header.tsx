...
export default function Header() {
  const { activePage, setActivePage, setCursorHovered, triggerSearch } = useAppStore();
  ...
  return (
    <header className="sticky top-0 z-50 bg-[#FDFBF7] border-b-2 border-black px-4 md:px-8 py-3 w-full flex items-center justify-between">
      ...
      >
        <span className="text-[#E23E26] font-syne font-extrabold text-3xl italic tracking-tighter uppercase">Go</span>
        <span className="text-black font-display font-extrabold text-3xl tracking-tight uppercase">Trip</span>
      </div>
      ...
      <nav className="hidden lg:flex items-center space-x-8 font-display text-sm font-semibold text-[#111111]">
        {navItems.map((item) => {
          const isActive = activePage === item.pageId || (item.pageId === 'amalfi' && activePage === 'hotel');
          return (
            <button
              key={item.label}
              ...
              className={`relative cursor-pointer uppercase transition-all duration-200 tracking-wider hover:text-[#E23E26] ${
                isActive ? 'text-[#E23E26] font-extrabold underline decoration-2 underline-offset-4' : 'text-[#222222]'
              }`}
            >
              {item.label}
            </button>
      ...
      <div className="flex items-center space-x-3 md:space-x-4">
        ...
        <form onSubmit={handleSearchSubmit} className="relative hidden md:flex items-center">
          ...
          />
          <button type="submit" className="absolute right-2 text-black cursor-pointer hover:text-[#E23E26]">
